...
as cost FROM country_club.Facilities


/* Q6: You'd like to get the first and last name of the last member(s)
who signed up. Do not use the LIMIT clause for your solution. */

SELECT surname, firstname , max(joindate) as lastjoined
...
SELECT DISTINCT mbr.surname AS member, facs.name AS facility
	FROM  `Members` mbr
	JOIN  `Bookings` bks ON mbr.memid = bks.memid
...
SELECT mbr.surname AS member, facs.name AS facility,
CASE
    WHEN mbr.memid = 0
